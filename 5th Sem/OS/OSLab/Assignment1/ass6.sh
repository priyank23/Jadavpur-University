...
echo
read -p "Enter string to replace: " str2
sed -i 's/\<'"$str"'\>/'"$str2"'/g' $File

numPartial=$(grep -o $str $File | wc -l)


if (( $numPartial == 0 )); then
	printf "$yellow No Partial matches exist!!$reset\n"
	exit 1
fi

printf "\nNumber of partial occurences: %d\n" $numPartial 
