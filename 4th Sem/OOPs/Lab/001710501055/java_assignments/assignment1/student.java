...
class student1
{
    ...
    void grade_s(int g)
    {
    if(g==10) System.out.println("S");
    if(g==9) System.out.println("A");
    if(g==8) System.out.println("B");
    if(g==7) System.out.println("C");
    if(g==6) System.out.println("D");
    if(g==5) System.out.println("E");
    if(g==4) System.out.println("X");
    if(g<4) {System.out.println("X");pass=0;}
    }
...
public class student extends student1
{
    ...
    int print_marksheet()
    {
        ...
        System.out.println("------------------------------------");
        if(pass==1) System.out.println("                    SGPA: "+sgpa/5);
        else System.out.println("                      SGPA:");
        System.out.println("------------------------------------");
        return 1;
    }
    static ArrayList<student> sort_list(ArrayList<student> s)
    {
    ...
    static int check_entry(ArrayList<student> s)
    {
...
{
    int[] count = new int[4];
    ArrayList<student>[] s = new ArrayList[4];
    University()
    {
        Arrays.fill(count, 0);
        for(int i = 0; i < 4; i++)
        { 
            s[i] = new ArrayList<student>(); 
        }
    } 
    void admin_interface() throws IOException
    {
        while(true)
        {
            ...
            switch(ch)
            {
                ...
                case 5:sorted_list(); break;
                case 6:show_num_stud();break;
                case 7:removeStud();break;
                case 8: System.gc();return;
                default: break;
            }
        }
    }
    ...
    void show_num_stud()
    {
        System.out.println("Total number of students who have taken admission is " + (s[0].size()+s[1].size()+s[2].size()+s[3].size()));
    }
    ...
    void rcv_marks()  throws IOException
    {
        ...
        while(true)
        {
            ...
            rl=bf.readLine();
            if(rl.length()!=9) System.out.println("Error! Try Again.");
            else break;
        }
        
        if("BCSE".equals(rl.substring(0,4))) dep_code=1;
        else if(rl.substring(0,4).equals("BCHE")) dep_code=2;
        else if(rl.substring(0,4).equals("ELEC")) dep_code=3;
        else if(rl.substring(0,4).equals("MECH")) dep_code=4;
        else
        {
        ...
        {
            student stud= s[dep_code-1].get(cnt-1);
            stud.set_marks();
        }
        else 
        {
    ...
    void marksheet()  throws IOException
    {
        ...
        if(rl.substring(0,4).equals("BCSE")) dep_code=1;
        else if(rl.substring(0,4).equals("BCHE")) dep_code=2;
        else if(rl.substring(0,4).equals("ELEC")) dep_code=3;
        else if(rl.substring(0,4).equals("MECH")) dep_code=4;
        else
        ...
        if(s[dep_code-1].size()>=cnt)
        {
            ...
            if(stud.roll.substring(8).equals("0")) return;
    ...
    void stu_list() throws IOException
    {
        ...
        System.out.print("\033[H\033[2J");
        System.out.flush();
        if(dpt==1) System.out.println("COMPUTER SCIENCE & ENGINEERING");
        if(dpt==2) System.out.println("CHEMICAL ENGINEERING");
        if(dpt==3) System.out.println("ELECTRICAL ENGINEERING");
        if(dpt==4) System.out.println("MECHANICAL ENGINEERING");
        for(int itr=0;itr<s[dpt-1].size();itr++)
        {
            student st=s[dpt-1].get(itr);
            st.show_stud(itr+1);
        }
    }  
    void sorted_list() throws IOException
    {
        ...
        if(dpt==3) System.out.println("ELECTRICAL ENGINEERING");
        if(dpt==4) System.out.println("MECHANICAL ENGINEERING");
        int flag=student.check_entry(s[dpt-1]);
        if(flag==1)
        {
        ArrayList<student> s_sorted = new ArrayList();
        s_sorted=student.sort_list(s[dpt-1]);
        for(int itr=0;itr<s[dpt-1].size();itr++)
        { 
            student st=s_sorted.get(itr);
            st.show_stud(itr+1);
        }
        }
        else
