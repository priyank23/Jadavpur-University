...
class Savings:public Person
{
	...
	public:
		...
		void initialize(float amt_todeposit)
		{
			...
			{
				cout<<"Should maintain min balance of 500"<<endl<<"Enter amount Again!"<<endl;
				float amt_depo;
				cin>>amt_depo;
				initialize(amt_depo);
			}
		...
		void withdrawal(float amt)
		{
			...
			else
			{
				...
				if(chk < 500)
				{
					...
					cout<<"Transaction NOT POSSIBLE\nShould maintain the minimum balance"<<endl<<endl;
				}
				else if(chk >= 500)
				{
					this->amt=chk;
		...
		void deposit(float amt)
		{
			...
			else
			{
				if(od_util==0)
				{
					...
					cout<<"OVERDRAFT utilized : "<<od_util<<endl<<endl;
				}
				else
				{
					if(amt>=od_util)
		        ...
		        if(ch==1)
		       	{
		            ...
		            if(flag==1)
		            {
		                ...
		                if(ans=='y' || ans=='Y')
		                {
		                	...
		                	otransaction<<save[index].getname()<<"\t\t"<<save[index].getAccNo()<<"\t\t"<<save[index].getamt()<<"\t\t"<<"DEPOSIT\t"<<date_update()<<"\t"<<time_update()<<endl;
		                }
		                else
		                {
		                	cout<<"1.DEPOSIT\n2.WITHDRAWAL\n3.BALANCE"<<endl;
		                	...
		                	else if(x==2)
		                	{
		                		...
		                		otransaction<<save[index].getname()<<"\t\t"<<save[index].getAccNo()<<"\t\t"<<save[index].getamt()<<"\t\t"<<"WITHDRAW\t"<<date_update()<<"\t"<<time_update()<<endl;
		                	}
		                	else if(x==3)
		                	{
		                		cout<<"YOUR BALANCE is : "<<save[index].getamt()<<endl<<endl;
		                	}
		                }
		            }
		            else
		            ...
		            if(flag==1)
		            {
		                ...
		                else
		                {
		                	...
		                	if(x==1)
		                	{
		                		...
		                		otransaction<<cur[index].getname()<<"\t\t"<<cur[index].getAccNo()<<"\t\t"<<cur[index].getamt()<<"\t\t"<<"DEPOSIT\t"<<date_update()<<"\t"<<time_update()<<endl;
		                	}
		                	else if(x==2)
		                	{
		                		cout<<"Enter Amount to Withdraw"<<endl;
		                		...
		                		otransaction<<cur[index].getname()<<"\t\t"<<cur[index].getAccNo()<<"\t\t"<<cur[index].getamt()<<"\t\t"<<"WITHDRAW\t"<<date_update()<<"\t"<<time_update()<<endl;
		                	}
		                	else if(x==3)
		                	{
		                		cout<<"YOUR BALANCE is : "<<cur[index].getamt()<<endl;
		                		cout<<"Overdraft Utilised : "<<cur[index].getodutil()<<endl<<endl;
		                	}
		                }
		            }
		            else
...
int main()
{
	...
	do
	{
		...
		cout<<"2.TO DO BALANCE QUERIES"<<endl<<endl;
		cin>>k1;
		if(k1==1)
			at.add_member();
		else if(k1==2)
